...
func update_shooting(delta: float) -> void:
	...
	burst_shot_interval_remaining = maxf(burst_shot_interval_remaining - delta, 0.0)

	if bullet_scene == null:
		return

	if burst_shots_remaining == 0 and shoot_cooldown_remaining > 0.0:
		return
	...
	if burst_shots_remaining == 0:
		burst_shots_remaining = shots_per_burst
		burst_shot_interval_remaining = 0.0

	if burst_shot_interval_remaining > 0.0:
		return
	...
	if burst_shots_remaining > 0:
		burst_shot_interval_remaining = burst_shot_interval_seconds
	else:
